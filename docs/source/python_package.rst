...
* overlay.py - used for managment of overlays. This includes checking status, loading overlays, and assignment of drivers

The PS module allows the Zynq PS control and stataus register to be read and set at runtime. Reading of the ARM CPU clock, and setting of the PL clocks is currently supported.
...
This folder contains modules for arduino shileds and peripherals that can be connected to an Arduino interface. This includes the arduino analog pins, and grove devices that can be connected through the PYNQ Grove shield. 
